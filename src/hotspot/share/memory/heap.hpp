...
class CodeHeap : public CHeapObj<mtCode> {
 ...
 protected:
  ...
  // Helper functions
  size_t   size_to_segments_down(size_t size) const { return size / _segment_size; }
  size_t   size_to_segments_up(size_t size) const { return (size + _segment_size -1) / _segment_size; }
  size_t   segments_to_size(size_t number_of_segments) const { return number_of_segments * _segment_size; }

  size_t   segment_for(void* p) const            { return ((char*)p - _memory.low()) / _segment_size; }
